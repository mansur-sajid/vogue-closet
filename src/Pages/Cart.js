...
const CartPage = () => {
  ...
  return (
    <div id="cartPage">
      <h1>Shopping Cart</h1>
      {cartItems.map((item) => (
        <div className="product d-flex col-md-5 col-10 offset-1 offset-md-3" key={item.id}>
          ...
          <div className="cart-details col-7">
            ...
            <Stack direction="row" className="mt-2" spacing={2}>
              <Button variant="contained" color="error" className="deletebutton" startIcon={<DeleteIcon/>} onClick={()=> dispatch(removeItem(item))}>
                Delete
              </Button>
            </Stack>
          </div>
      ...
      <Button variant="contained" color="error" className="clearCart" startIcon={<DeleteIcon/>} onClick={()=> dispatch(clearCart())}>
                Clear Cart
              </Button>
    </div>
  );
